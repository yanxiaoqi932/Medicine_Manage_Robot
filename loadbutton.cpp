...
loadbutton::loadbutton(QString loadpath)
{
    loadButtonPath = loadpath;  //按钮图片路径
    QPixmap pixmap;
    bool ret = pixmap.load(loadButtonPath);
    if(!ret)
    {
        qDebug()<<loadpath<<"加载图片失败！";
    }
